...
Overlay deste repositório. Nem toda correção merece um PRD. Nem toda correção dispensa registro. Esta skill decide qual.
...
3. **Registre na SPEC existente**, em `## 2. Requisitos Técnicos`, como subseção de correção — com a data e o que estava errado. Adicione o item correspondente à DoD. Se a SPEC já estava `[x] Implementada`, volte o Status para `[ ] Em Progresso`: ela só volta a `Implementada` quando o `quality-assurance-validation-agent` aprovar o item novo.
...
Pare a trilha leve e chame o `product-owner-prd-agent` quando a correção:
