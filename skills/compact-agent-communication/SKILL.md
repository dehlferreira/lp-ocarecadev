...
| `Aceite` | comando + resultado | `npm test → 0 fail` |
| `Riscos/Bloqueios` | o que pode quebrar, o que falta | `Safari <17 sem animation-timeline: degrada estático.` |
| `Próximo responsável` | nome do agente | `quality-assurance-validation-agent` |
...
> `Aceite` npm run build → ok. npm test → 0 fail. CLS manual iPhone 390px → 0.00.
> `Riscos/Bloqueios` Não testado em Android Chrome.
> `Próximo responsável` quality-assurance-validation-agent
</Good>

O primeiro tem 61 palavras e uma afirmação não verificável ("acredito que está tudo funcionando"). O segundo tem fatos checáveis.
