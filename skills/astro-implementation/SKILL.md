...
Overlay deste repositório, depois das skills do template (`frontend-implementation-execution`). Menor mudança coesa que satisfaz a SPEC. Nada além dela.
...
| `src/assets/images/`       | assets otimizados (WebP), entregues pelo `image-creator-agent` |
...
| **G7** | dependência nova = pare e devolva ao `teachlead-architecture-agent` |
...
| Marcar a DoD como concluída                      | Só o `quality-assurance-validation-agent` marca  |
| Dizer "tudo funcionando" sem rodar os comandos   | Rodar os três e colar o resultado                |

## Handoff

Arquivos alterados, comando + resultado, regressões manuais verificadas, limitações → `quality-assurance-validation-agent`, no formato de `skills/compact-agent-communication/SKILL.md`. Teste que falhou vai em texto completo, não comprimido.
