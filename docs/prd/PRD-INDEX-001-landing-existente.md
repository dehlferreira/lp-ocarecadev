# PRD-INDEX-001 — Landing OCARECADEV existente

Índice de produto já aceito. Não recrie estes PRDs. Feature nova continua em `PRD-007`.

## Origem

Documentos migrados de `docs/prds/` para `docs/prd/` em 2026-09-01, com histórico git preservado. Conteúdo e regras de negócio **não** foram reescritos.

## PRDs

| Arquivo | Tema |
| --- | --- |
| `docs/prd/PRD-001-visao-geral-e-negocios.md` | posicionamento, três planos, público, jornada |
| `docs/prd/PRD-002-design-e-interface.md` | interface, glassmorphism, hierarquia visual |
| `docs/prd/PRD-003-animacoes-e-scroll.md` | movimento e scroll |
| `docs/prd/PRD-004-marketing-e-tracking.md` | marketing, consentimento, tracking |
| `docs/prd/PRD-005-arquitetura-e-performance.md` | performance, SEO, restrições de stack |
| `docs/prd/PRD-006-copy-final-e-secoes-faltantes.md` | copy e seções |

Copy oficial: `docs/referencias/copy-completa-landing.md`.

## Próximo responsável

Mudança de escopo → `product-owner-prd-agent`. Implementação já coberta → `docs/specs/SPEC-INDEX-001-landing-existente.md`.
